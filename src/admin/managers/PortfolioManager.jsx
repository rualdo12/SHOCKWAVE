...
const empty = { title: '', description: '', image: '', tags: '' };
...
const PortfolioManager = () => {
  ...
  useEffect(() => {
    if (!db) return;
    const unsub = listenToCollection('portfolio', setItems);
    return () => unsub && unsub();
  }, []);
  ...
  const save = async () => {
    ...
    try {
      if (editing === 'new') {
        const ref = await createItem('portfolio', payload);
        logActivity({ type: 'portfolio:create', uid: user?.uid, email: user?.email, id: ref.id });
      } else {
        await updateItem('portfolio', editing, payload);
        logActivity({ type: 'portfolio:update', uid: user?.uid, email: user?.email, id: editing });
      }
      setEditing(null);
      setForm(empty);
      setStatus('Saved');
  ...
  const remove = async (id) => {
    ...
    logActivity({ type: 'portfolio:delete', uid: user?.uid, email: user?.email, id });
  };
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {items.map((item) => (
          <div key={item.id} className="glass-card border border-gray-800 rounded-xl p-4 space-y-2">
            ...
            {item.tags && <p className="text-xs text-gray-400">Tags: {(item.tags || []).join(', ')}</p>}
          </div>
        ))}
      </div>
    </ManagerShell>
