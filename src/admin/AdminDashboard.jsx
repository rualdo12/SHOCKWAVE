      ...
      {logs.map((log) => (
        <div key={log.id} className="glass-card border border-gray-800 rounded-xl p-4 flex justify-between gap-3">
          <div className="space-y-1">
            <p className="text-xs uppercase tracking-[0.2em] text-custom-yellow">{log.type || 'event'}</p>
  ...
  return (
    <div className="min-h-screen bg-custom-dark text-white">
      ...
      <header className="relative border-b border-gray-800 bg-black/70 backdrop-blur px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-sm text-custom-yellow uppercase tracking-[0.3em]">Admin</p>
        ...
        <div className="flex items-center gap-3">
          <a
            href="/"
            className="px-4 py-2 rounded border border-gray-700 text-gray-200 hover:border-custom-yellow hover:text-white transition"
          >
            Preview Site
          </a>
          <button
            onClick={logout}
            className="px-4 py-2 rounded border border-custom-yellow text-custom-yellow hover:bg-custom-yellow hover:text-black transition font-bold"
          >
            Logout
          </button>
        </div>
      ...
      <main className="relative container mx-auto px-4 py-8">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg border transition ${
                active === t.key
                  ? 'border-custom-yellow text-black bg-custom-yellow shadow-[0_0_20px_rgba(253,190,51,0.3)]'
                  : 'border-gray-700 text-gray-300 hover:border-custom-yellow hover:text-white bg-black/40 backdrop-blur'
              }`}
            >
              {t.label} Manager
            </button>
        ...
        <div className="glass-card border border-custom-yellow/20 rounded-2xl p-6">{renderTab()}</div>
      </main>
    </div>
  );
};
